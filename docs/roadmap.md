...
| 2026-06-24 | 追加     | リファレンス性強化。**Custom Hooks をテスト容易性のために切り出し**（`useTicketTableState`=URL同期, `useTicketForm`=フォーム状態）→ TicketTable/TicketForm を薄く。フックは next/navigation をモックしブラウザ無しでVitest単体テスト。`docs/design-guide.md`（責務分離の解説）を追加。62テスト、Stmt100/Branch94.7/Func98.5/Line100% |
| 2026-06-25 | 追加     | **作成ダイアログ + IF設計**。`FormDialog`(汎用枠/composition) + `TicketCreateDialog`(ドメイン) + `TicketForm` に `onSuccess`(ok シグナル)を追加し**ページ/ダイアログ両用**に。一覧の新規作成をダイアログ化。design-guide に「IF設計のケーススタディ」追記。E2E(Playwright)で 作成→閉じる→一覧更新 を確認。68テスト。functions閾値は v8 の "use server" 過小計数のため85（他90） |
